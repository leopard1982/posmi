    ...
    if request.user.is_authenticated:
        if request.user.is_superuser:
            try:
                ...
                file.write('no,id_transaksi,nomor_nota,cabang,user_id,user_name,total_item,total_bayar,metode_bayar,tanggal,customer,nota_print,kuitansi_print,alasan_void\n')
                ...
                for jual in penjualan:
                    if jual.is_void:
                        ...
                        alasan_void="-"
                    file.write(f"{index},{jual.nota},{int(jual.no_nota):05d},{jual.cabang.prefix},{jual.user.username},{jual.user.userprofile.nama_lengkap},{jual.items},{jual.total},{jual.metode},{jual.tgl_bayar.strftime('%d/%h/%Y %H:%M:%Y')},{jual.customer},{jual.reprint_nota},{jual.cetak_kuitansi},{alasan_void}\n")
                    index +=1
                file.close()
